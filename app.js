...
async function loadCmnts(rid,btn,force){
  ...
  w.innerHTML=`<div style="margin-top:12px;padding-top:12px;border-top:1px solid var(--b1)">
    ${(cs||[]).map(c=>{const u=uMap[c.user_id];return`<div style="padding:8px 10px;background:var(--bg3);border-radius:8px;margin-bottom:5px"><div style="font-size:10px;font-weight:700;color:var(--accent2);margin-bottom:2px">@${u?.username||'user'}</div><div style="font-size:13px;color:var(--text2)">${c.text}</div></div>`;}).join('')}
...
async function addCmnt(rid){
  ...
  const{error}=await sb.from('rating_comments').insert({rating_id:rid,user_id:CU.id,text:t});
