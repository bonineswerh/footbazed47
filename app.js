...
function renderNav(){
  ...
  if(CU){
    ...
    const cls=avColor(n);
    nr.innerHTML=`<div class="notif-btn" id="notifBtn" onclick="toggleNotif()" style="display:flex">${ico('bell',18)}<div class="notif-badge" id="notifBadge"></div></div><div class="nav-user" onclick="go('profile')"><div class="nav-av ${cls}">${n[0].toUpperCase()}</div><span class="nav-uname">${n}</span></div><button class="nbtn nbtn-ghost" onclick="doLogout()">Выйти</button>`;
  }else{
...
async function addFriend(fid){
  ...
  try{await sb.from('notifications').insert({user_id:fid,from_user_id:CU.id,type:'friend_request',message:`${CU.display_name||CU.username||'Кто-то'} хочет дружить`});}catch(e){}
...
function editProfile(){
  const w=document.getElementById('profileW');
  const cls=avColor(CU.display_name||'x');
  w.innerHTML=`
  <div class="phero" style="max-width:500px;margin:0 auto">
    <div class="phero-av ${cls}" style="margin-bottom:20px">${(CU.display_name?.[0]||'U').toUpperCase()}</div>
    <h2 style="font-family:'Bebas Neue',sans-serif;font-size:24px;margin-bottom:20px">${ico('edit',20)} Редактирование профиля</h2>
    <div style="width:100%;max-width:380px">
      <label style="font-size:12px;color:var(--text2);display:block;margin-bottom:4px;text-align:left">Имя</label>
      <input class="input" id="ep_name" value="${(CU.display_name||'').replace(/"/g,'&quot;')}" placeholder="Имя">
      <label style="font-size:12px;color:var(--text2);display:block;margin-bottom:4px;text-align:left">Никнейм</label>
      <input class="input" id="ep_user" value="${(CU.username||'').replace(/"/g,'&quot;')}" placeholder="Никнейм">
      ...
      <div style="display:flex;gap:10px;margin-top:8px">
        ...
        <button class="btn btn-l" style="flex:1" onclick="saveEditProfile()">Сохранить</button>
      </div>
    </div>
  </div>`;
}
async function saveEditProfile(){
  ...
  if(user)upd.username=user;
  if(bio!==undefined)upd.bio=bio||null;
  if(teams!==undefined)upd.favorite_teams=teams||null;
  const{error}=await sb.from('users').update(upd).eq('id',CU.id);
  if(error){toast('Ошибка: '+error.message,'err');return;}
  CU.display_name=name;CU.username=user||CU.username;CU.bio=bio;CU.favorite_teams=teams;
