...
async function loadRateP(mid){
  ...
  // Search each team separately with multiple strategies
  const hp=await findTeamPlayers(m.home_team_name);
  const ap=await findTeamPlayers(m.away_team_name);
...
async function findTeamPlayers(teamName){
  // Strategy 1: exact match
  let{data:ps}=await sb.from('players').select('*').eq('team',teamName);
  if(ps?.length)return ps;

  // Strategy 2: ilike with full name
  ({data:ps}=await sb.from('players').select('*').ilike('team','%'+teamName+'%'));
  if(ps?.length)return ps;

  // Strategy 3: try main word from team name (remove FC, CF, AFC etc)
  const cleaned=teamName.replace(/\b(FC|CF|AFC|SC|SSC|AC|AS|RC|CD|UD|SD|RCD|CA|SE|SL|US|BSC|TSG|VfB|VfL|1\.)\b/gi,'').trim();
  if(cleaned&&cleaned!==teamName){
    ({data:ps}=await sb.from('players').select('*').ilike('team','%'+cleaned+'%'));
    if(ps?.length)return ps;
  }

  // Strategy 4: first significant word (at least 4 chars)
  const words=teamName.split(/\s+/).filter(w=>w.length>=4&&!/^(FC|CF|AFC|SC|Club|City|United|Town|County|Real)$/i.test(w));
  if(words.length){
    ({data:ps}=await sb.from('players').select('*').ilike('team','%'+words[0]+'%'));
    if(ps?.length)return ps;
  }
  return[];
}
...
function renderTeamSquad(teamName,players){
  ...
  const posGroup={'GK':'gk','CB':'def','LB':'def','RB':'def','Defence':'def','DM':'mid','CM':'mid','AM':'mid','LM':'mid','RM':'mid','Midfield':'mid','LW':'att','RW':'att','ST':'att','Offence':'att','Centre-Forward':'att','Centre-Back':'def','Left-Back':'def','Right-Back':'def','Defensive Midfield':'mid','Central Midfield':'mid','Attacking Midfield':'mid','Left Winger':'att','Right Winger':'att','Goalkeeper':'gk'};
