...
const TEAM_ALIASES={
  ...
  'wolves':'wolverhampton','wolverhampton':'wolverhampton wanderers',
  'atleti':'atletico madrid','atletico':'atletico madrid',
  'barca':'barcelona','barça':'barcelona',
  'real':'real madrid','bayern':'bayern munich','bayern münchen':'bayern munich',
  'dortmund':'borussia dortmund','bvb':'borussia dortmund',
  'leverkusen':'bayer leverkusen','bayer 04':'bayer leverkusen',
  'gladbach':'borussia monchengladbach','bmg':'borussia monchengladbach',
  'inter':'inter milan','inter milan':'fc internazionale milano',
  'ac milan':'ac milan','milan':'ac milan',
  'juve':'juventus','napoli':'ssc napoli',
  ...
  'newcastle':'newcastle united','villa':'aston villa','aston villa':'aston villa',
  'brighton':'brighton and hove albion','crystal palace':'crystal palace',
  'west ham':'west ham united','nottm forest':'nottingham forest','nott forest':'nottingham forest',
  'leicester':'leicester city','everton':'everton fc',
  'sevilla':'sevilla fc','betis':'real betis','sociedad':'real sociedad',
  'villarreal':'villarreal cf','bilbao':'athletic club','athletic':'athletic club',
  'freiburg':'sc freiburg','mainz':'1. fsv mainz 05','frankfurt':'eintracht frankfurt',
  'hoffenheim':'tsg hoffenheim','augsburg':'fc augsburg','stuttgart':'vfb stuttgart',
  'lazio':'ss lazio','roma':'as roma','fiorentina':'acf fiorentina',
  'atalanta':'atalanta bc','bologna':'bologna fc','torino':'torino fc',
  'lens':'rc lens','rennes':'stade rennais','nice':'ogc nice','nantes':'fc nantes',
  'strasbourg':'rc strasbourg','reims':'stade de reims','montpellier':'montpellier hsc',
};
...
function normalizeTeamName(name){
  ...
  return name.toLowerCase()
    .replace(/[''`]/g,'')
    .replace(/\b(fc|cf|afc|sc|ssc|ac|as|rc|cd|ud|sd|rcd|ca|se|sl|us|bsc|tsg|vfb|vfl|1\.|sc)\b/gi,'')
    .replace(/\s+/g,' ').trim();
}

function matchTeam(matchTeamName,playerTeamName){
  const mn=matchTeamName.toLowerCase().trim();
  const pn=playerTeamName.toLowerCase().trim();

  // Exact match
  if(mn===pn)return 100;

  // Direct contains
  if(pn.includes(mn)||mn.includes(pn))return 90;

  // Alias match
  const alias=TEAM_ALIASES[mn];
  if(alias&&(pn.includes(alias)||alias.includes(pn)||pn.includes(alias.split(' ')[0])))return 85;

  // Normalized match
  const nmn=normalizeTeamName(mn);
  const npn=normalizeTeamName(pn);
  if(npn.includes(nmn)||nmn.includes(npn))return 80;

  // Word overlap: count how many words match
  const mWords=mn.split(/[\s\-]+/).filter(w=>w.length>=3);
  const pWords=pn.split(/[\s\-]+/).filter(w=>w.length>=3);
  let matchCount=0;
  for(const mw of mWords){
    for(const pw of pWords){
      if(pw.includes(mw)||mw.includes(pw))matchCount++;
    }
  }
  ...
  // Alias word match
  if(alias){
    const aWords=alias.split(/[\s\-]+/).filter(w=>w.length>=3);
    let ac=0;
    for(const aw of aWords){for(const pw of pWords){if(pw.includes(aw)||aw.includes(pw))ac++;}}
    if(ac>0)return 40+ac*10;
  }

  return 0;
}

function findPlayersForTeam(allPlayers,teamName){
  // Get all unique teams and find best match
  const teamMap={};
  allPlayers.forEach(p=>{if(!teamMap[p.team])teamMap[p.team]=[];teamMap[p.team].push(p);});

  let bestTeam=null,bestScore=0;
  for(const pt of Object.keys(teamMap)){
    const score=matchTeam(teamName,pt);
    if(score>bestScore){bestScore=score;bestTeam=pt;}
  }

  if(bestScore>=40&&bestTeam)return teamMap[bestTeam];
  return[];
}
...
async function loadRateP(mid){
  ...
  if(!hp.length&&!ap.length){
    el.innerHTML='<div style="color:var(--fog);font-size:12px;margin-bottom:14px">Игроки не найдены для этого матча</div>';
    return;
  }
